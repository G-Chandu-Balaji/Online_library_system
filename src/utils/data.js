export const Books = [
  {
    id: 1,
    author: "George Orwell",
    title: "1984",
    description:
      "A dystopian novel set in a totalitarian society ruled by Big Brother.",
    image: "https://m.media-amazon.com/images/I/71kxa1-0mfL.jpg",
    category: "Dystopian Fiction",
    year: 1949,
  },
  {
    id: 2,
    author: "J.K. Rowling",
    title: "Harry Potter and the Sorcerer's Stone",
    description:
      "The first book in the beloved Harry Potter series, following Harry's first year at Hogwarts.",
    image: "https://m.media-amazon.com/images/I/81YOuOGFCJL.jpg",
    category: "Fantasy",
    year: 1997,
  },
  {
    id: 3,
    title: "The Great Gatsby",
    author: "F. Scott Fitzgerald",
    category: "Classic Fiction",
    year: 1925,
    description:
      "A critique of the American Dream set in the Roaring Twenties.",
    image: "https://m.media-amazon.com/images/I/41oAprmWTSL._SY445_SX342_.jpg",
  },
  {
    id: 4,
    author: "Michelle Obama",
    title: "Becoming",
    description:
      "A deeply personal memoir by the former First Lady of the United States.",
    image: "https://m.media-amazon.com/images/I/81h2gWPTYJL.jpg",
    category: "Biography",
    year: 2018,
  },
  {
    id: 5,
    author: "Yuval Noah Harari",
    title: "Sapiens: A Brief History of Humankind",
    description:
      "A thought-provoking exploration of the history and impact of Homo sapiens.",
    image: "https://m.media-amazon.com/images/I/41yu2qXhXXL._SY445_SX342_.jpg",
    category: "History",
    year: 2011,
  },
  {
    id: 6,
    author: "Harper Lee",
    title: "To Kill a Mockingbird",
    description:
      "A Pulitzer Prize-winning novel set in the racially segregated South during the 1930s.",
    image: "https://m.media-amazon.com/images/I/41j-s9fHJcL._SY445_SX342_.jpg",
    category: "Fiction",
    year: 1960,
  },
  {
    id: 7,
    author: "J.R.R. Tolkien",
    title: "The Hobbit",
    description:
      "A fantasy novel that introduces readers to the world of Middle-earth.",
    image: "https://covers.openlibrary.org/b/id/14627222-L.jpg",
    category: "Fantasy",
    year: 1937,
  },
  {
    id: 8,
    author: "Dan Brown",
    title: "The Da Vinci Code",
    description:
      "A gripping thriller that combines history, art, and religion in a search for hidden secrets.",
    image:
      "https://ia600100.us.archive.org/view_archive.php?archive=/5/items/l_covers_0012/l_covers_0012_39.zip&file=0012392336-L.jpg",
    category: "Thriller",
    year: 2003,
  },
  {
    id: 9,
    author: "J.R.R. Tolkien",
    title: "The Fellowship of the Ring",
    description:
      "The first book in the epic Lord of the Rings trilogy, filled with adventure and friendship.",
    image:
      "https://upload.wikimedia.org/wikipedia/en/8/8e/The_Fellowship_of_the_Ring_cover.gif",
    category: "Fantasy",
    year: 1954,
  },
  {
    id: 10,
    author: "Markus Zusak",
    title: "The Book Thief",
    description:
      "A story of a young girl in Nazi Germany who steals books to survive during the war.",
    image: "https://m.media-amazon.com/images/I/81OthjkJBuL.jpg",
    category: "Historical Fiction",
    year: 2005,
  },
  {
    id: 11,
    author: "John Green",
    title: "The Fault in Our Stars",
    description:
      "A poignant romance about two young cancer patients falling in love with each other.",
    image: "https://covers.openlibrary.org/b/id/7285167-L.jpg",
    category: "Romance",
    year: 2012,
  },
  {
    id: 12,
    author: "F. Scott Fitzgerald",
    title: "Tender is the Night",
    description:
      "A tragic love story set in the glamour and heartbreak of the French Riviera in the 1920s.",
    image:
      "https://thebigsmoke.com.au/wp-content/uploads/tender-is-the-night.jpg",
    category: "Classic Literature",
    year: 1934,
  },
  {
    id: 13,
    author: "Stephen King",
    title: "It",
    description:
      "A chilling horror novel about a group of friends confronting a malevolent force in their childhood town.",
    image:
      "https://ia600505.us.archive.org/view_archive.php?archive=/35/items/l_covers_0014/l_covers_0014_65.zip&file=0014651795-L.jpg",
    category: "Horror",
    year: 1986,
  },
  {
    id: 14,
    author: "Suzanne Collins",
    title: "The Hunger Games",
    description:
      "A dystopian novel where teenagers must participate in a deadly televised competition.",
    image: "https://covers.openlibrary.org/b/id/14855544-L.jpg",
    category: "Dystopian Fiction",
    year: 2008,
  },
  {
    id: 15,
    author: "Isaac Asimov",
    title: "Foundation",
    description:
      "The first book in a classic science fiction series about the rise and fall of civilizations.",
    image:
      "https://ia600505.us.archive.org/view_archive.php?archive=/35/items/l_covers_0014/l_covers_0014_61.zip&file=0014612980-L.jpg",
    category: "Science Fiction",
    year: 1951,
  },
  {
    id: 16,
    author: "Agatha Christie",
    title: "Murder on the Orient Express",
    description:
      "A detective novel featuring Hercule Poirot, solving a murder on a luxurious train.",
    image:
      "https://ia600505.us.archive.org/view_archive.php?archive=/35/items/l_covers_0014/l_covers_0014_56.zip&file=0014561973-L.jpg",
    category: "Mystery",
    year: 1934,
  },
  {
    id: 17,
    author: "Haruki Murakami",
    title: "Norwegian Wood",
    description:
      "A novel about love, loss, and mental health, set in 1960s Tokyo.",
    image:
      "https://ia801909.us.archive.org/view_archive.php?archive=/31/items/l_covers_0013/l_covers_0013_61.zip&file=0013613167-L.jpg",
    category: "Literary Fiction",
    year: 1987,
  },
  {
    id: 18,
    author: "Chimamanda Ngozi Adichie",
    title: "Americanah",
    description:
      "A story about love, identity, and race, set between Nigeria and the United States.",
    image:
      "https://ia800100.us.archive.org/view_archive.php?archive=/5/items/l_covers_0012/l_covers_0012_62.zip&file=0012627263-L.jpg",
    category: "Contemporary Fiction",
    year: 2013,
  },
  {
    id: 19,
    author: "Neil Gaiman",
    title: "American Gods",
    description:
      "A dark fantasy novel about the struggle between old and new gods in modern America.",
    image:
      "https://ia800505.us.archive.org/view_archive.php?archive=/35/items/l_covers_0014/l_covers_0014_42.zip&file=0014420908-L.jpg",
    category: "Fantasy",
    year: 2001,
  },
  {
    id: 20,
    author: "Paulo Coelho",
    title: "The Alchemist",
    description:
      "A philosophical novel about a young shepherd’s journey to find his personal legend.",
    image: "https://covers.openlibrary.org/b/id/14828473-L.jpg",
    category: "Philosophy",
    year: 1988,
  },
];
